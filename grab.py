...
def main():
	try:
		print(f" {reset}\033[4mSite Scraper{reset}")
		Cubdo().liat()
		for i in range(1,19):
		...
		if menu:
			outs = input(f" {reset}Mau simpan di mana?? > {cyan}")
...
class Cubdo:
	...
	def gasken(self):
		try:
			...
			print(f"{reset} Anjay capek gw bro, makasih sudah pakai gw\n")
		except KeyboardInterrupt:
			print(f"{reset} killed!")
			exit()
